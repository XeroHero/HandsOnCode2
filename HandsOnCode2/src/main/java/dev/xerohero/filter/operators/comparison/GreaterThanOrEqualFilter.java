...
/**
 * A filter that checks if a resource's value is greater than or equal to a specified value.
 */
public class GreaterThanOrEqualFilter extends BaseComparisonFilter {

    /**
     * Creates a new greater-than-or-equal filter.
     *
     * @param key   The key to check in the resource
     * @param value The value that the resource's value should be greater than or equal to
     * @throws IllegalArgumentException if key is null or empty, or if value is not a valid number
     */
    public GreaterThanOrEqualFilter(String key, String value) {
        super(key, value);
    }
    ...
    public boolean matches(Map<String, String> resource) {
        ...
        // If the key doesn't exist or is null, it's not greater than or equal
        if (actualValue == null) {
            return false;
        }
